...
window.setupCallEvents = function(call) {
    call.on('stream', rs => {
        ...
        // IMPORTANT: Never remove an existing wrapper when the same peer renegotiates.
        // Replacing the DOM node was causing the Host to briefly lose guest rendering
        // during reconnects/track changes. Reuse the wrapper and only replace its stream.
        let w = document.getElementById(wid);
        let v = w ? w.querySelector('video') : null;

        if (!w) {
            w = document.createElement('div');
            w.id = wid;
            videoLayer.appendChild(w);
        }
        ...
        const playRemote = () => {
            v.muted = false;
            v.volume = 1.0;
            if (v.paused) v.play().catch(e => console.warn('Remote audio/video autoplay blocked:', e));
        };
        v.onloadedmetadata = playRemote;
        v.oncanplay = playRemote;
        playRemote();

        let nameTag = w.querySelector('.video-name-tag');
        if (!nameTag) {
        ...
        let spotlightBtn = w.querySelector('.spotlight-btn');
        if (!spotlightBtn) {
            spotlightBtn = document.createElement('button');
...
function setupAudioVisualizer(stream, wrapperId, isRemote) {
    try {
        ...
        if(isRemote) {
            // Keep remote voice on the native media element for clean, full-volume playback.
            // WebAudio is used only for the speaking indicator; the old HRTF/panner path
            // could make speech sound distant or quiet on mobile devices.
...
function leaveRoom() { 
    ...
    if (ytIframe) ytIframe.src = `https://www.youtube.com/embed/${DEFAULT_YOUTUBE_VIDEO_ID}?autoplay=0&controls=1&rel=0&playsinline=1`;
    currentYtVideoId = DEFAULT_YOUTUBE_VIDEO_ID;
    currentYtTime = 0;
    currentYtState = 2; 
...
const dataChannelHandlers = {
    ...
    'hello': (msg, conn) => {
        ...
        let currentTime = currentYtTime || 0;
        let currentState = currentYtState || 2;
    ...
    'magnet-off': (msg, conn) => {
        if (!isHost) {
            window.isGuestMagnetized = false;
            showMagnetApp(false);
            if (currentActiveStage === 'magnetLayer') {
                switchMainStage(magnetPreviousGuestStage || 'videoLayer');
            }
            logSystemMsg("MAGNET OFF: The Magnet app was removed. Your workspace is unchanged.");
        }
    },
    ...
    'magnet-on': (msg, conn) => {
        ...
        window.magnetizedStageId = msg.stage || 'youtubeLayer';
        updateMagnetStage(window.magnetizedStageId);
        showMagnetApp(true);
        if (msg.ytVid) applyMagnetYoutubeState({vidId:msg.ytVid, time:msg.ytTime || 0, state:msg.ytState || 2, sentAt:msg.sentAt || Date.now()});
        logSystemMsg("MAGNET ON: Open the new Magnet app to view the Host. Your own workspace stays untouched.");
    },

    'magnet-youtube-state': (msg, conn) => {
        if (!isHost && window.isGuestMagnetized) applyMagnetYoutubeState(msg);
    },
    ...
    'room-sync': (msg, conn) => {
        ...
        if (msg.magnetOn && !isHost) {
            ...
            updateMagnetStage(window.magnetizedStageId);
            showMagnetApp(true);
            if (msg.ytVid) applyMagnetYoutubeState({vidId:msg.ytVid, time:msg.ytTime || 0, state:msg.ytState || 2, sentAt:Date.now()});
        }
        // Magnet is a separate guest view: never hijack the guest's own workspace.
        if (msg.stage && !(msg.magnetOn && !isHost)) switchMainStage(msg.stage);
        ...
        if (!isHost && msg.ytVid) {
            applyIncomingYouTube({vidId: msg.ytVid, time: msg.ytTime || 0, state: msg.ytState || 2});
        }
            ...
            roomMembers.forEach(m => {
                if (m.id !== peer.id && !m.isHost && peer.id > m.id) {
                    ...
                    if (!document.getElementById(wid) && !outgoingMediaCalls[m.id]) {
                        ...
                        outgoingMediaCalls[call.peer] = call;
                        call.on('stream', rs => {
                            // Render guest-to-guest media without registering a second stream
                            // listener on the same PeerJS call.
                            ...
                            if (!w || !v) {
                                ...
                                wrap.id = wid;
                                wrap.className = window.peerCamStates[call.peer] === false ? 'video-wrapper video-off' : 'video-wrapper';
                                v = document.createElement('video');
                                v.id = call.peer;
                                v.autoplay = true;
                                v.playsInline = true;
   ...
   'cam-state': (msg, conn) => { 
    ...
    // YouTube is intentionally API-free. Keep lightweight room state only.
    'yt-master-sync': (msg, conn) => applyIncomingYouTube(msg),
    'yt-play': (msg, conn) => applyIncomingYouTube({...msg, state: 1}),
    'yt-pause': (msg, conn) => applyIncomingYouTube({...msg, state: 2}),
    'yt-heartbeat': (msg, conn) => applyIncomingYouTube(msg),
...
// ==========================================
// 10. MEDIA, BROWSERS, & YOUTUBE — NO API VERSION
// ==========================================
...
let currentYtVideoId = DEFAULT_YOUTUBE_VIDEO_ID;
let currentYtTime = 0;
let currentYtState = 2;
let hostYtPlayer = null;
let magnetYtPlayer = null;
let magnetYtReady = false;
let hostYtReady = false;
let magnetLastSentAt = 0;
let magnetSyncTimer = null;
let applyingMagnetState = false;
let pendingMagnetYoutubeState = null;
let magnetPreviousGuestStage = 'videoLayer';

function getYouTubeVideoId(input) {
    const value = String(input || '').trim();
    if (!value) return null;
    const regex = /(?:youtube\.com\/(?:watch\?(?:.*&)?v=|embed\/|shorts\/|live\/|v\/)|youtu\.be\/)([A-Za-z0-9_-]{11})/i;
    const match = value.match(regex);
    if (match) return match[1];
    return /^[A-Za-z0-9_-]{11}$/.test(value) ? value : null;
}

function youtubeEmbedUrl(videoId, startTime = 0, autoplay = false) {
    const id = getYouTubeVideoId(videoId) || DEFAULT_YOUTUBE_VIDEO_ID;
    const start = Math.max(0, Math.floor(Number(startTime) || 0));
    const origin = encodeURIComponent(window.location.origin);
    return `https://www.youtube.com/embed/${encodeURIComponent(id)}?autoplay=${autoplay ? 1 : 0}&controls=1&rel=0&playsinline=1&enablejsapi=1&origin=${origin}&start=${start}`;
}

function setYouTubeIframe(videoId, startTime = 0, autoplay = false) {
    const iframe = document.getElementById('ytIframe');
    const empty = document.getElementById('ytEmptyState');
    const id = getYouTubeVideoId(videoId) || DEFAULT_YOUTUBE_VIDEO_ID;
    const start = Math.max(0, Math.floor(Number(startTime) || 0));
    currentYtVideoId = id;
    currentYtTime = start;
    currentYtState = autoplay ? 1 : 2;
    if (empty) empty.style.display = 'none';

    if (isHost && hostYtPlayer && hostYtReady) {
        hostYtPlayer.loadVideoById({videoId:id, startSeconds:start});
        if (!autoplay) hostYtPlayer.pauseVideo();
        return;
    }
    if (iframe) iframe.src = youtubeEmbedUrl(id, start, autoplay);
}
...
function openDirectYouTube() {
    ...
    if (iframe && (!iframe.src || iframe.src === 'about:blank')) {
        setYouTubeIframe(DEFAULT_YOUTUBE_VIDEO_ID, 0, false);
    }
    setTimeout(initYouTubePlayers, 50);
}

function broadcastYtVideo(videoId) {
    if (!isHost) {
    ...
    currentYtState = 2;
    switchMainStage('youtubeLayer');
    setYouTubeIframe(id, 0, false);
    magnetBroadcast({type:'magnet-youtube-state', vidId:id, time:0, state:2, seq:Date.now()});
    if (typeof addResourceToCabinet === 'function') addResourceToCabinet('YouTube Video', 'youtube', `https://youtube.com/watch?v=${id}`, 'fab fa-youtube');
    broadcastData({ type: 'yt-load', vidId: id, time: 0, state: 2 });
...
function triggerYouTubeSync() {
    ...
    const id = getYouTubeVideoId(input ? input.value : '');
    if (!id) {
        alert('Paste a valid YouTube video link or 11-character video ID.');
        return;
    }
    broadcastYtVideo(id);
...
function handleYtSync(c, hostTime) {
    if (typeof hostTime === 'number') currentYtTime = hostTime;
    currentYtState = c === 'play' ? 1 : 2;
}

function applyIncomingYouTube(msg) {
    if (isHost || !msg) return;
    const id = getYouTubeVideoId(msg.vidId);
    if (!id) return;
    currentYtVideoId = id;
    currentYtTime = Number(msg.time) || 0;
    currentYtState = Number(msg.state) || 2;
    switchMainStage('youtubeLayer');
    setYouTubeIframe(id, currentYtTime, currentYtState === 1);
}

/* ==========================================================
   MAGNET APP — HOST IS THE SOURCE OF TRUTH
   Guests see a read-only mirror of the Host's current activity.
   The Guest's own workspace is never switched or controlled.
   ========================================================== */
function getMagnetApp() { return document.getElementById('magnetApp'); }

window.isGuestMagnetized = window.isGuestMagnetized || false;
window.magnetizedStageId = window.magnetizedStageId || 'youtubeLayer';

function showMagnetApp(show = true) {
    const tile = document.getElementById('magnetAppTile');
    if (tile) tile.style.display = (!isHost && show) ? 'block' : 'none';
}

function openMagnetStage() {
    if (isHost || !window.isGuestMagnetized) return;

    if (currentActiveStage !== 'magnetLayer') {
        magnetPreviousGuestStage = currentActiveStage || 'videoLayer';
    }

    updateMagnetStage(window.magnetizedStageId || 'youtubeLayer');
    switchMainStage('magnetLayer');

    // This click is a real user gesture. Apply the latest Host state immediately
    // so mobile browsers have the best chance of allowing playback with audio.
    if (pendingMagnetYoutubeState) {
        const pending = pendingMagnetYoutubeState;
        pendingMagnetYoutubeState = null;
        applyMagnetYoutubeState(pending, true);
    } else if (window.magnetLatestYoutubeState) {
        applyMagnetYoutubeState(window.magnetLatestYoutubeState, true);
    }
}

function closeMagnetStage() {
    if (!isHost && window.isGuestMagnetized && currentActiveStage === 'magnetLayer') {
        switchMainStage(magnetPreviousGuestStage || 'videoLayer');
    }
}

function updateMagnetStage(stage) {
    const yt = document.getElementById('magnetYoutubeView');
    const generic = document.getElementById('magnetGenericView');
    const label = document.getElementById('magnetGenericStage');
    const status = document.getElementById('magnetAppStatus');
    const isYt = stage === 'youtubeLayer';
    const friendly = String(stage || 'activity')
        .replace('Layer', '')
        .replace(/([A-Z])/g, ' $1')
        .trim();

    if (yt) yt.style.display = isYt ? 'block' : 'none';
    if (generic) generic.style.display = isYt ? 'none' : 'flex';
    if (label) label.textContent = `Host is using ${friendly}`;
    if (status) status.textContent = isYt ? 'Mirroring Host YouTube' : `Mirroring Host: ${friendly}`;
}

function magnetBroadcast(msg) {
    if (!isHost) return;
    broadcastData(msg);
}

function getHostYouTubeState() {
    let time = Number(currentYtTime) || 0;
    let state = Number(currentYtState);

    if (hostYtPlayer && hostYtReady) {
        try { time = Number(hostYtPlayer.getCurrentTime()) || 0; } catch (_) {}
        try { state = Number(hostYtPlayer.getPlayerState()); } catch (_) {}
    }

    if (![0, 1, 2, 3, 5].includes(state)) state = 2;
    currentYtTime = time;
    currentYtState = state;
    return { time, state };
}

function sendMagnetYoutubeState(force = false) {
    if (!isHost || !window.magnetMode || currentActiveStage !== 'youtubeLayer') return;
    if (!hostYtReady || !hostYtPlayer) return;

    const now = Date.now();
    // 250ms gives a responsive mirror while keeping DataChannel traffic small.
    if (!force && now - magnetLastSentAt < 250) return;
    magnetLastSentAt = now;

    const media = getHostYouTubeState();
    const msg = {
        type: 'magnet-youtube-state',
    ...
    // Cache the exact state too, so a newly opened Guest Magnet can immediately
    // display the latest known Host position.
    window.magnetLastYoutubeBroadcast = msg;
    magnetBroadcast(msg);
}

function applyMagnetYoutubeState(msg, fromUserGesture = false) {
    if (isHost || !msg) return;

    window.magnetLatestYoutubeState = msg;

    if (!magnetYtPlayer || !magnetYtReady) {
        pendingMagnetYoutubeState = msg;
        return;
    }
    ...
    const previousSeq = Number(window.magnetLastAppliedSeq) || 0;
    if (seq && seq < previousSeq) return;
    if (seq) window.magnetLastAppliedSeq = seq;

    const hostState = Number(msg.state);
    const hostTime = Math.max(0, Number(msg.time) || 0);
    const elapsed = hostState === 1 && Number.isFinite(Number(msg.sentAt))
        ? Math.max(0, (Date.now() - Number(msg.sentAt)) / 1000)
        : 0;
    const target = hostTime + elapsed;

    applyingMagnetState = true;
    try {
        let currentId = '';
        try {
            currentId = String(magnetYtPlayer.getVideoData().video_id || '');
        } catch (_) {}

        if (currentId !== id) {
            magnetYtPlayer.loadVideoById({ videoId: id, startSeconds: target });
        } else {
            const localTime = Number(magnetYtPlayer.getCurrentTime()) || 0;
            // Correct drift, but don't constantly seek a normally-playing video.
            if (Math.abs(localTime - target) > 0.25) {
                magnetYtPlayer.seekTo(target, true);
            }
        }

        // Host is authoritative. Guest has no Magnet player controls.
        if (hostState === 1 || hostState === 3) {
            magnetYtPlayer.playVideo();
        } else if (hostState === 2) {
            magnetYtPlayer.pauseVideo();
            // A paused Host must remain at the Host's exact paused position.
            setTimeout(() => {
                if (!magnetYtPlayer || !magnetYtReady) return;
                try {
                    const local = Number(magnetYtPlayer.getCurrentTime()) || 0;
                    if (Math.abs(local - hostTime) > 0.20) magnetYtPlayer.seekTo(hostTime, true);
                    magnetYtPlayer.pauseVideo();
                } catch (_) {}
            }, 40);
        } else if (hostState === 0) {
            magnetYtPlayer.seekTo(hostTime, true);
            magnetYtPlayer.pauseVideo();
        }
    } catch (e) {
        console.warn('Magnet YouTube sync failed:', e);
    }

    setTimeout(() => { applyingMagnetState = false; }, 120);

    const badge = document.getElementById('magnetSyncBadge');
    if (badge) {
        const label = hostState === 1 ? 'PLAYING' : hostState === 2 ? 'PAUSED' : hostState === 0 ? 'ENDED' : 'LIVE';
        badge.textContent = `HOST ${label} • ${target.toFixed(2)}s`;
    }

    // If playback was blocked by mobile autoplay policy, retry only from a
    // genuine Guest click (openMagnetStage), never by stealing the Guest's work.
    if (fromUserGesture && (hostState === 1 || hostState === 3)) {
        try { magnetYtPlayer.playVideo(); } catch (_) {}
    }
}
...
function initYouTubePlayers() {
    ...
    const hostFrame = document.getElementById('ytIframe');
    if (hostFrame && !hostYtPlayer) {
        hostYtPlayer = new YT.Player('ytIframe', {
            events: {
                onReady: function() {
                    hostYtReady = true;
                    if (isHost && window.magnetMode) sendMagnetYoutubeState(true);
                },
                onStateChange: function(e) {
                    if (!isHost || applyingMagnetState) return;
                    currentYtState = Number(e.data);
                    try { currentYtTime = Number(hostYtPlayer.getCurrentTime()) || 0; } catch (_) {}
                    // Play/pause/seek changes are sent immediately.
                    if (window.magnetMode && currentActiveStage === 'youtubeLayer') {
                        sendMagnetYoutubeState(true);
                    }
                }
            }
        });
    ...
    const magnetFrame = document.getElementById('magnetYtIframe');
    if (magnetFrame && !magnetYtPlayer) {
        magnetYtPlayer = new YT.Player('magnetYtIframe', {
            playerVars: {
                controls: 0,
                disablekb: 1,
                rel: 0,
            ...
            events: {
                onReady: function() {
                    magnetYtReady = true;
                    // Never let a Guest's local Magnet interaction become source data.
                    try { magnetYtPlayer.setVolume(100); } catch (_) {}
                    const latest = pendingMagnetYoutubeState || window.magnetLatestYoutubeState;
                    pendingMagnetYoutubeState = null;
                    if (latest && window.isGuestMagnetized) {
                        applyMagnetYoutubeState(latest, false);
                    }
                },
                onStateChange: function() {
                    // Magnet is read-only. Host state is always authoritative.
                    if (!applyingMagnetState && window.magnetLatestYoutubeState) {
                        setTimeout(() => {
                            if (window.magnetLatestYoutubeState && window.isGuestMagnetized) {
                                applyMagnetYoutubeState(window.magnetLatestYoutubeState, false);
                            }
                        }, 0);
                    }
                }
...
window.onYouTubeIframeAPIReady = function() {
    setTimeout(initYouTubePlayers, 0);
};

function startMagnetHeartbeat() {
    clearInterval(magnetSyncTimer);
    magnetSyncTimer = setInterval(() => {
        if (isHost && window.magnetMode && currentActiveStage === 'youtubeLayer') {
            sendMagnetYoutubeState(false);
        }
    }, 250);
}

// HOST MAGNET: toggle only the separate Guest Magnet app. It never navigates
// Guests away from their own Grid/Canvas/Development/etc.
function forceSyncStage() {
    if (!isHost) return;

    window.magnetMode = !window.magnetMode;
    ...
    if (window.magnetMode) {
        ...
        // Tell every current Guest immediately.
        broadcastData(magnetMsg);

        if (currentActiveStage === 'youtubeLayer') {
            sendMagnetYoutubeState(true);
        }

        if (typeof playBoardSound === 'function') playBoardSound('chime', false);
        logSystemMsg('MAGNET ON: Guests get a read-only live view of your activity. Their workspace stays independent.');
    } else {
        broadcastData({ type: 'magnet-off', seq: Date.now() });
        if (typeof playBoardSound === 'function') playBoardSound('chime', false);
        logSystemMsg('MAGNET OFF: The Guest Magnet app is removed.');
    }
}

// When the Host changes apps, update only the Guest's Magnet source.
const _originalSwitchMainStage = switchMainStage;
switchMainStage = function(s) {
    _originalSwitchMainStage(s);

    if (isHost && window.magnetMode) {
        const media = s === 'youtubeLayer' ? getHostYouTubeState() : {time:0, state:2};
        broadcastData({
            type: 'magnet-on',
            stage: s,
            ytVid: currentYtVideoId,
        ...
        if (s === 'youtubeLayer') {
            setTimeout(() => sendMagnetYoutubeState(true), 80);
        }
    }
};

document.addEventListener('click', () => {
    if (window.YT && YT.Player) initYouTubePlayers();
}, {passive:true});

document.addEventListener('DOMContentLoaded', () => {
    const minBtn = document.getElementById('magnetMinimizeBtn');
    if (minBtn) minBtn.onclick = () => closeMagnetStage();
    startMagnetHeartbeat();
    initYouTubePlayers();
});
